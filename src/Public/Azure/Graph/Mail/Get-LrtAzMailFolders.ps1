...
Function Get-LrtAzMailFolders {
    ...
    Process {
        ...
        $RequestUri = "https://graph.microsoft.com/v1.0/users/$UserPrincipalName/mailFolders"
        ...
        if ($Received) {
            $_received = $Received.ToString("MM/dd/yyyy")
            if ($Search) {
                $RequestUri += "&`"received:$_received`""
            } else {
                $RequestUri += "?`$search=`"received:$_received`""
            }
        }
        ...
        if ($HasAttachment) {
            if ($Search) {
                $RequestUri += "&`"hasAttachments:true`""
            } else {
                $RequestUri += '?$search="hasAttachments:true"'
            }
        }
