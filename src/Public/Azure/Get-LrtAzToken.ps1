...
Function Get-LrtAzToken {
    ...
    .DESCRIPTION
        The Get-LrtAzToken cmdlet returns an access token for Microsoft Azure protected
        resources. The access token returned by this cmdlet is a an object containing
        the token itself, expiry information, and a URL to the resource endpoint.

        LogRhythm.Tools's implementation of Microsoft Azure utilizes the Azure Active
        Directory (v1.0) endpoint to request access tokens, via OAuth2.

        Because the LogRhythm.Tools module does not act on behalf of users, 
        dynamic/incremental consent considerations are not necessary. Additionally, the
        Microsoft Identity Platform endpoint doesn't support all Azure AD scenarios and
    ...
    .PARAMETER ResourceName
        The name of the resource for which to obtain a token. This can either be "AzureAD"
        or "DefenderATP", which are the two supported Azure resourced in LogRhythm.Tools
    .PARAMETER Credential
        A PSCredential for an Azure Application Client containing the Client Id and 
        Client Secret in the UserName and Password properties, respectively. By default,
        this value is retrieved from the LogRhythm.Tools configuration file generated by
        Setup.ps1
    .INPUTS
    ...
    .EXAMPLE
        ...
        PS C:\> $Token = Get-LrtAzToken -Credential $Credential

        Description
        -----------
        Deserialize an encrypted PSCredential and returns an Azure access token to $Token.
    .LINK
    ...
    Process {
        ...
        switch ($ResourceName) {
            "AzureAd" {
                ...
                $OAuth2Uri = $LrtConfig.AzureAD.OAuth2Url
            }
            "DefenderATP" { 
                $ResourceUri = $LrtConfig.DefenderATP.Resource
                $OAuth2Uri = $LrtConfig.DefenderATP.OAuth2Url
            }
        }
        ...
        Write-Verbose "OAuth2Url: $OAuth2Uri"
