...
function Install-Lrt {
    ...
    switch ($true) {
        ...
        $IsLinux { 
            ...
            if ($Scope -eq "System") {
                # Need to qualify means to validate *nix permissions of user for System install
            }
            #endregion
            ...
            if (! (Test-Path -Path $ScopeInfo.Path)) {
                if ($Scope -eq "User") {
                    ...
                    $_created = New-Item -Path $HOME -Name ".local/share/powershell/Modules" `
            ...
            if ($ScopeInfo.Installed) {
                ...
                if ($NewestVersion -gt $InstallerVersion) {
                    ...
                    # Silent install for automated install support
